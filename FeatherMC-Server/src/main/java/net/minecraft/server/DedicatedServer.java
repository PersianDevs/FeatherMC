...
//        DedicatedServer.LOGGER.info("Loading properties"); // FeatherMC - removed useless logs
        this.propertyManager = new PropertyManager(this.options); // CraftBukkit - CLI argument support

        // FeatherMC Start - removed EULA
...
//        } else {
        ...
        // FeatherMC End
            ...
            this.setMotd(this.propertyManager.getString("motd", "A FeatherMC Server"));
...
//            DedicatedServer.LOGGER.info("Default game type: " + this.r); // FeatherMC - removed useless logs
...
//            DedicatedServer.LOGGER.info("Generating keypair");  // FeatherMC - removed useless logs
            this.a(MinecraftEncryption.b());
            DedicatedServer.LOGGER.info("Dar Hale Start Kardane Server Dar IP Va Porte: " + (this.getServerIp().length() == 0 ? "*" : this.getServerIp()) + ":" + this.getPort()); // Nacho - deobfuscate getPort // FeatherMC

        if (!org.spigotmc.SpigotConfig.lateBind) {
            try {
                this.aq().a(inetaddress, this.getPort()); // Nacho - deobfuscate getPort
            } catch (IOException ioexception) {
                DedicatedServer.LOGGER.warn("Khata Dar Bind Kardane Port");
                DedicatedServer.LOGGER.warn("Khata In Bood: {}", ioexception.toString());
                DedicatedServer.LOGGER.warn("Fek Konam Serveret Az Ghabl Ba In Porte Runne?");
                return false;
            }
        }
        ...
        // FeatherMC - Removed useless logs
         ...
         */
            ...
            if (!NameReferencingFileConverter.a(this.propertyManager)) {
                ...
                DedicatedServer.LOGGER.info("Dar Hale Load Kardane Worlde \"" + this.U() + "\"");
                this.a(this.U(), this.U(), k, worldtype, s2);
                long i1 = System.nanoTime() - j;
                String s3 = String.format("%.3fs", (double) i1 / 1.0E9D);

                DedicatedServer.LOGGER.info("Server Run Shod (" + s3 + ")! Baraye Help, \"help\" or \"?\" Ro Type Konid.");
...
//        server.getLogger().info( "**** Beginning UUID conversion, this may take A LONG time ****"); // FeatherMC - removed useless logs
        boolean flag = false;
