...
public final class CraftServer implements Server {
    ...
    private String serverName = "FeatherMC";
        ...
        if (pluginFolder.exists()) {
            ...
            for (Plugin plugin : plugins) {
                try {
                    ...
                    plugin.getLogger().info(String.format("Dar Hale Load Kardane Plugine %s", plugin.getDescription().getFullName()));
                    plugin.onLoad();
                } catch (Throwable ex) {
    ...
    public String getServerName() {
        return this.getConfigString("server-name", "FeatherMC");
    }

    @Override
    public String getServerId() {
        return this.getConfigString("server-id", "FeatherMC");
    }
...
//                        System.out.println("Preparing spawn area for " + name + ", " + (j1 * 100 / i1) + "%"); // FeatherMC
                        i = l;
                    }
